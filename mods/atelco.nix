      ...
      lib.nameValuePair "atelco-${name}" {
        ...
        serviceConfig = {
          ...
          ExecStartPre = [
            ...
            "${lib.getExe' pkgs.execline "umask"} 0007 ${lib.getExe' pkgs.coreutils "touch"} ${cfg.database}"
          ];
          ExecStart = "${lib.getExe cfg.package} ${name} --database ${cfg.database} ${socket} ${mkFlags value}";
        };
      })
    cfg.daemons;
  };
